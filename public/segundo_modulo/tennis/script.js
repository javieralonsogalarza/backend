...
function cropImage(image, boxes, expand) {
  ...
  // Ajuste para evitar la distorsi¨®n manteniendo el aspecto original
...
document.getElementById("imageUpload").addEventListener("change", async (event) => {
  ...
  // We'll no longer disable the button, just show the loading indicator
  
  // Limpiar la imagen del localStorage para evitar cargar una imagen anterior al refrescar
  const partidoId = datos['partido'];
  if (partidoId) {
    const storageKey = `imagen_partido_${partidoId}`;
    localStorage.removeItem(storageKey); // Eliminar la imagen anterior del localStorage
  }
  ...
  image.onload = async () => {
    ...
    if (detections.length > 0) {
      ...
      // We no longer need to enable/disable the button
      ...
      li.innerHTML = `
        ...
        <p>Foto cargada correctamente! <br>Se encontraron <b>${detections.length} rostros</b>. Adem¨¢s, se considera(n) <b>${croppedData.distance}</b> del encuadre de la foto.</p>
      `;
