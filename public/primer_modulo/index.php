...
<body>
    ...
    function procesarNombresJugadores(&$datos) {
        ...
        if (isset($datos['ganador']) && !empty($datos['ganador'])) {
            ...
            $datos['ganador'] = cortarNombreApellido($datos['ganador'], $nombrePilaGanador, 29, true);
        }
                    ...
                    foreach ($datos['llaves'][$ronda] as $bloque => &$partidos) {
                        foreach ($partidos as &$partido) {
                            ...
                            if (isset($partido['jugador_local']) && !empty($partido['jugador_local'])) {
                                ...
                                $partido['jugador_local'] = cortarNombreApellido($partido['jugador_local'], $nombrePilaLocal, 29, true);
                            }
                            ...
                            if (isset($partido['jugador_rival']) && !empty($partido['jugador_rival'])) {
                                ...
                                $partido['jugador_rival'] = cortarNombreApellido($partido['jugador_rival'], $nombrePilaRival, 29, true);
                            }
                        }
                    }
                }
    ...
    $datos =  $datos['multiple'] ?  $datos : procesarNombresJugadores($datos);
    ...
    <?php if (!empty($datos) && $validacion > 2) : ?>
        ...
        if (isset($datos['ronda']) && !empty($datos['ronda'])) {
            if ($datos['ronda'] == 64) {
                $start_number = 16;  // Nuevo valor para ronda de 64
            } elseif ($datos['ronda'] == 32) {
                $start_number = 8;
        ...
        <div class="canvas_scroll w-100">
            <div class="canvas ig <?php echo $canvas_class ?? ''; ?>" data-first-round="<?php echo $start_number; ?>" id="diagram">
                ...
                <?php if ($start_number != 1) : ?>
                    ...
                    <?php if (isset($llaves['ronda64']) && !empty($llaves['ronda64'])) : ?>
    <?php 
    $bloque_map = [
        "bloque_uno" => 0,
        "bloque_dos" => 1,
    ...
    foreach ($bloque_map as $bloque => $index) {
        if (isset($llaves['ronda64'][$bloque])) {
            $partidos_ordenados[$index] = $llaves['ronda64'][$bloque];
        }
    }
    ?>

    <div class="canvas_col-16">
        <?php 
        $n = 0;
        foreach ($partidos_ordenados as $index => $partidos) : 
...
<?php endif; ?>
                ...
                <?php if (isset($datos['ganador']) && !empty($datos['ganador'])) : ?>
                    <div class="canvas_winner absolute">
                        <img src="https://laconfraternidaddeltenis.com/upload/image/trofeo.png">
