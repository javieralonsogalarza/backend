...
function cortarNombreApellido($nombreCompleto, $maxCaracteres = 15) {
    // 1. Extraer el paréntesis con el número, incluyendo ESPACIOS OPCIONALES antes
    preg_match('/\(\d+\)/', $nombreCompleto, $parentesisMatch);
    $parentesis = isset($parentesisMatch[0]) ? $parentesisMatch[0] : '';

    // 2. Calcular la longitud del nombre *SIN* el paréntesis
    $nombreSinParentesis = str_replace($parentesis, '', $nombreCompleto);
    
    // Usar mb_strlen para manejar caracteres especiales
    $longitudNombre = mb_strlen(trim($nombreSinParentesis), 'UTF-8');

    if ($longitudNombre <= $maxCaracteres) {
        return $nombreCompleto;
    }

    // 3. Recortar el nombre *SIN* el paréntesis
    // Usar mb_substr para cortar correctamente
    $nombreCortado = mb_substr(trim($nombreSinParentesis), 0, $maxCaracteres, 'UTF-8');

    // 4. Agregar el paréntesis al nombre recortado
    $resultado = trim($nombreCortado) . ' ' . $parentesis;
            ...
            <div class="canvas canvas-full bg<?php echo numeroAleatorio(); ?>">
                ...
                <div class="canvas_body">
                    ...
                    <?php if (!empty($datos['jugadores'])): ?>
                        ...
                        $cols = 2;
                        $class = 'separado';
                        $cantidad_jugadores = $datos['multiple'] ? count($datos['jugadores']) *2 : count($datos['jugadores']);
...
<?php foreach ($datos['jugadores'] as $jugador) {
    if(isset($datos['multiple']) && $datos['multiple'] == true){
        $nombreV1 = cortarNombreApellido($jugador['nombresv1'] . ' ' . $jugador['apellidosv1']);
        $nombreV2 = cortarNombreApellido($jugador['nombresv2'] . ' ' . $jugador['apellidosv2']);
