            ...
            if (!$Validator->fails()){
                ...
                if($entity != null)
                {
                    ...
                    if(count($Categorias) > 0)
                    {
                        ...
                        {
                         $entity->torneoCategorias()->where('id', $q->id)->update([ 'first_final' => $first_final]);
                    ...
                    if($request->categorias != null)
                    {
                        foreach ($request->categorias as $key => $q) {
                            $categorias = explode("-", $q);
                            if(count($categorias) == 1){$ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => null, 'orden' => $key, 'sector' => $i, 'multiple' => false, 'user_create_id' => Auth::guard('web')->user()->id, 'first_final' =>$first_final];}
                            else{$ToneoCategorias[] = ['torneo_id' => $entity->id, 'categoria_simple_id' => $categorias[0], 'categoria_dupla_id' => $categorias[1], 'orden' => $key, 'sector' => $i, 'multiple' => true, 'user_create_id' => Auth::guard('web')->user()->id, 'first_final' =>$first_final];}
                            $i++;
                        }
                    }
                }
    ...
    public function finish(Request $request)
    {
        ...
        try {
            ...
            if($TorneoCategoria != null)
            {
               ...
               $TorneoCategoria->estado_id = $TorneoCategoria->estado_id == App::$ESTADO_FINALIZADO 
    ? App::$ESTADO_FINALIZADO 
    : ($request->finalizar == "true" || $request->finalizar == true 
        ? App::$ESTADO_FINALIZADO 
        : App::$ESTADO_PENDIENTE);
...
return $TorneoFaseFinal->JugadoresClasificados;
}
        }
    }
    ...
    public function faseFinalStore(Request $request)
    {
       ...
       //CLASIFICADOS POR CÁLCULO
           // Inicializar arrays para almacenar los lugares
                ...
                $partidosvalidar = Partido::where('torneo_categoria_id', $request->torneo_categoria_id)
                ->where('fase',6)->where('estado_id', App::$ESTADO_PENDIENTE)
                ->get();

          

                if(count($partidosvalidar)==6){
                    //actualizar fase deesos aprtidos a 4 
                    Partido::where('torneo_id', $TorneoCategoria->torneo_id)->where('torneo_categoria_id', $TorneoCategoria->id)
                    ->where('fase',6)->where('estado_id', App::$ESTADO_PENDIENTE)->update(['fase' => 4]);
                }
...
->first();
    ...
    public function grupo($id, $torneo_categoria_id = null, $fase = null, $landing = false)
    {
        ...
        $torneoCategoria = $entity->torneoCategorias()->where('id', $torneo_categoria_id)->with('categoriaSimple')->orderBy('orden')->first();
...
$TorneoFaseFinal = (object)[
    ...
    'JugadoresClasificados' => App::multiPropertySort(
        ...
        if(count($TorneoFaseFinal->JugadoresClasificados) > 0
        &&
        (
        ...
        $affectedRows = Jugador::query()->update(['nombre_completo_temporal' => DB::raw('NULL')]);
    ...
    public function grupoTablaPartialView($id, $torneo_categoria_id, $grupo_id, $landing=false,$visual=true)
    {
        $ComunidadId = $landing ? Comunidad::where('principal', true)->first()->id : Auth::guard('web')->user()->comunidad_id;
            ...
            if (!$Validator->fails())
            {
                ...
                if($Torneo != null)
                {
                    if(count($Torneo->partidos->where('torneo_categoria_id', $request->torneo_categoria_id)) <= 0)
                    {
                        ...
                        if((count($Jugadores) % 4) == 0 || $request->tipo == "zonas")
                        {
                            ...
                            // Find duplicates and their zones
                            $jugadoresPorId = $allJugadores->groupBy('jugador_simple_id');
                            $duplicados = $jugadoresPorId->filter(function($grupo) {
                                return $grupo->count() > 1;
                            });
                            
                            // Group by zone and convert to array
                            $JugadoresPorZona = $allJugadores->groupBy('zona_id')
                                ->map(function($grupo) use ($duplicados, $allJugadores) {
                                    return $grupo->filter(function($jugador) use ($duplicados, $grupo, $allJugadores) {
                                        if (!$duplicados->has($jugador->jugador_simple_id)) {
                                            return true;
                                        }
                                        
                                        $zonasDelJugador = collect();
                                        foreach ($duplicados[$jugador->jugador_simple_id] as $duplicado) {
                                            $zonasDelJugador->push([
                                                'zona_id' => $duplicado->zona_id,
                                                'count' => $allJugadores->where('zona_id', $duplicado->zona_id)->count()
                                            ]);
                                        }
                                        
                                        $minCount = $zonasDelJugador->min('count');
                                        return $grupo->count() == $minCount;
                                    })->values(); // Convert to array and reset keys
                                });
                            
                          $Jugadores = $JugadoresPorZona;
...
foreach ($Jugadores as $zonaId => $jugadoresZona) {
    ...
    if ($jugadoresZona->count() % 4 !== 0) {
        ...
        $jugadoresNecesarios = 4 - ($jugadoresZona->count() % 4);
        ...
        if ($jugadoresTemporales->count() < $jugadoresNecesarios) {
            $Result->Message = "No hay suficientes jugadores temporales disponibles para completar la zona {$zonaNombre}. Se necesitan {$jugadoresNecesarios} jugadores temporales.";
            return response()->json($Result);
        }

        // Crear registros de torneo_jugador para los jugadores temporales
        foreach ($jugadoresTemporales as $jugadorTemporal) {
            ...
            // Agregamos el jugador temporal a la colección de jugadores de esta zona
            $jugadoresZona->push($torneoJugador);
        }
    }
}
...
while ($jugadoresZona->isNotEmpty()) {
                            ...
                            foreach ($Grupos as $q)
                            {
                                ...
                                if($CantidadPartidos > 0)
                                {
                                    ...
                                    if(count($PartidosModel) > 0)
                                    {
                                        ...
                                        // los partidos que tienen jugadores temporal osea en jugadors temporal=true deberia tener estado_id en finalizado

                                        $PartidosTemporales = Partido::where('torneo_id', $TorneoCategoria->torneo_id)
                                        ->where('torneo_categoria_id', $TorneoCategoria->id)
                                        ->where('grupo_id', $q->id)
                                        ->where('estado_id', App::$ESTADO_PENDIENTE)
                                        ->where(function($query) {
                                            $query->whereHas('jugadorLocalUno', function($q) {
                                                $q->where('temporal', true);
                                            })
                                            ->orWhereHas('jugadorRivalUno', function($q) {
                                                $q->where('temporal', true);
                                            });
                                        })
                                        ->get();
                                    
                                    foreach ($PartidosTemporales as $q2) {
                                        $q2->update(['estado_id' => App::$ESTADO_FINALIZADO]);
                                    }
    ...
    public function grupoDelete(Request $request)
    {
        ...
        try {
            ...
            if($TorneoCategoria != null)
            {
                ...
                //eliminar jugadores temporales que estan en la tabla torneo_jugadors pero solo los jugadores temporales no todos
                
                TorneoJugador::where('torneo_categoria_id', $TorneoCategoria->id)
                ->where('torneo_id', $TorneoCategoria->torneo_id)
                ->whereHas('jugadorSimple', function($query) {
                    $query->where('temporal', true); // Filtrar por jugadores temporales en la tabla jugadors
                })
                ->delete();
                DB::commit();
                $Result->Success = true;
    ...
    public function partidoStore(Request $request)
    {
        ...
        try {
           ...
           $jugadores =   $this->faseFinal($request->torneo_id, $request->torneo_categoria_id,false, false);
                          ...
                          if(false)
                          {
                              if(Partido::where('torneo_categoria_id', $TorneoCategoria->id)->whereNotNull('fase')->count() > 0)
                              {
                                ...
                                if(TorneoCategoria::where('torneo_id', $request->torneo_id)->where('comunidad_id', Auth::guard('web')->user()->comunidad_id, 'manual',1)){
                                    // Volver a llamar a la función grupoTablaPartialView
                                $nuevosJugadores = $this->faseFinal($request->torneo_id, $request->torneo_categoria_id, false, false);
                               
                                    
                             
                                // Actualizar los partidos en la base de datos
                                foreach ($jugadores as $index => $jugador) {

                                    $validarResultado = Partido::where('torneo_id', $request->torneo_id)
                                    ->where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
                                    ->where('torneo_categoria_id', $request->torneo_categoria_id)
                                    ->where('estado_id', App::$ESTADO_FINALIZADO)
                                    ->where(function($query) use ($jugador) {
                                        $query->where('jugador_local_uno_id', $jugador['jugador_simple_id'])
                                              ->orWhere('jugador_rival_uno_id', $jugador['jugador_simple_id']);
                                    })
                                    ...
                                    ->count();
                                        ...
                                        if($validarResultado == 0){
                                            // Actualizar los partidos donde el jugador es local
                                          Partido::where('torneo_id', $request->torneo_id)
                                                ->where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
                                                ->where('torneo_categoria_id', $request->torneo_categoria_id)
                                                ->where('estado_id', App::$ESTADO_PENDIENTE)
                                                ->where('jugador_local_uno_id', $jugador['jugador_simple_id'])
                                                ->where('jugador_rival_uno_id', '!=', $nuevosJugadores[$index]['jugador_simple_id'])
                                                ->update(['jugador_local_uno_id' => $nuevosJugadores[$index]['jugador_simple_id']]);
                                          
                                            // Actualizar los partidos donde el jugador es rival
                                            Partido::where('torneo_id', $request->torneo_id)
                                                ->where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
                                                ->where('torneo_categoria_id', $request->torneo_categoria_id)
                                                ->where('estado_id', App::$ESTADO_PENDIENTE)
                                                ->where('jugador_rival_uno_id', $jugador['jugador_simple_id'])
                                                ->where('jugador_local_uno_id', '!=', $nuevosJugadores[$index]['jugador_simple_id'])
                                                ->update(['jugador_rival_uno_id' => $nuevosJugadores[$index]['jugador_simple_id']]);
                                        }
    ...
    public function partidoGenerateJsonv2(Request $request)
    {
        $content = "";
...
public function partidoGenerateJson(Request $request)
{
    try {
        ...
        if($Partido != null)
        {
            ...
            $ResultadoGanador = $Partido->resultado; 
            $ResultadoPerdedor = '';
            if(!in_array($Partido->resultado, ["wo", "w.o", "WO", "W.O"]))
            {
                $ResultadoLeft = []; 
            ...
            // Construir la ruta de la imagen usando la misma lógica de uploadImageSegunda
            $torneoFolder = $this->sanitizeFilename($Partido->torneoCategoria->torneo->nombre) . '-' . $Partido->torneo_categoria_id;
            $categoriaFolder = $this->sanitizeFilename($Partido->torneoCategoria->categoriaSimple->nombre) . '-' . $Partido->torneoCategoria->categoriaSimple->id;
            
            // Implementar la misma lógica para determinar el nombre de la ronda
            $rondaNumero = $Partido->fase ? $Partido->fase : 0;
            $nombreRonda = '';
            ...
            // Usar el mismo formato para el nombre de la ronda
            $rondaFolder = $nombreRonda . '-' . $rondaNumero;
            
            // Usar el mismo formato para el nombre del grupo y partido
            $grupoFolder = $TorneoGrupo ? $this->sanitizeFilename($TorneoGrupo->nombre_grupo) : null;
            $partidoFolder = 'partido-' . $Partido->id;

            // Construir la ruta de la carpeta usando el mismo formato
            ...
            if (Storage::disk('public')->exists($folderPath)) {
                ...
                if (!empty($files)) {
                    $imagePath = Storage::url($files[0]); // Obtener la primera imagen encontrada
                }
            }
            ...
            $model = (object)[
                ...
                'grupo' => $TorneoGrupo != null ? $TorneoGrupo->nombre_grupo : null,
                'ronda' => $Partido->fase ?? 'Fase de grupos',
                'categoria' => $Partido->multiple && ($Partido->torneoCategoria->categoriaSimple->id !== $Partido->torneoCategoria->categoriaDupla->id) 
                    ? ($Partido->torneoCategoria->categoriaSimple->nombre." + ".$Partido->torneoCategoria->categoriaDupla->nombre) 
                    : ($Partido->torneoCategoria->categoriaSimple->nombre),
                'jugador_ganador_uno' => $Partido->jugadorGanadorUno->nombre_completo_temporal,
                'jugador_ganador_dos' => $Partido->multiple ? ($Partido->jugadorGanadorDos->nombre_completo) : null,
                'jugador_rival_uno'  => $Partido->jugador_local_uno_id == $Partido->jugador_ganador_uno_id 
                    ? $Partido->jugadorRivalUno->nombre_completo_temporal 
                    : $Partido->jugadorLocalUno->nombre_completo_temporal,
                'jugador_rival_dos'  => $Partido->multiple 
                    ? ($Partido->jugador_local_dos_id == $Partido->jugador_ganador_dos_id 
                        ? $Partido->jugadorRivalDos->nombre_completo 
                        : $Partido->jugadorLocalDos->nombre_completo) 
                    : null,
                'resultado_ganador'  => $ResultadoGanador,
                'resultado_rival' => $ResultadoPerdedor,
                'partido' => $Partido->id,
        ...
        Log::error('Error en partidoGenerateJson: ' . $e->getMessage());
    ...
    public function ranking($torneo, $torneo_categoria_id, $landing=false)
    {
        ...
        if($TorneoCategoria != null)
        {
                ...
                foreach ($Jugadores as $q2)
                {
                    if($TorneoCategoria->multiple)
                    {
                        ...
                        foreach ($PartidosComoLocal as $p)
                        {
                            ...
                            {   //NO Rival
                                $SetsGanados += $p->jugador_local_set; $SetsPerdidos += $p->jugador_rival_set;
                                $GamesGanados += $p->jugador_local_juego; $GamesPerdidos += 0;
                                $Puntos += ($p->jugador_local_set == 0 && $p->jugador_rival_set == 0 ? 0 : ($p->jugador_rival_set <= 0 ? 5 : 4));
                            }else{
                                //Rival
                                $SetsGanados += $p->jugador_rival_set; $SetsPerdidos += $p->jugador_local_set;
            ...
            //traerme torneocategoria 
                ...
                $partidos=  Partido::where('torneo_categoria_id', $TorneoCategoria->id)->whereNull('deleted_at')->where(['estado_id' => App::$ESTADO_FINALIZADO])->get();
   ...
   public function exportJugadorJson(Request $request)
    {
        ...
        if($TorneoCategoria != null)
        {
            ...
            $jugadoresSinNumero = [];
                ...
                {
                $jugadorSimple = $q->jugadorSimple;
                $jugadorDupla = $q->jugadorDupla;

                $Jugadores['nombresv1'] = $TorneoCategoria->multiple ? $jugadorSimple->nombres : $jugadorSimple->nombres;
                $Jugadores['apellidosv1'] = $TorneoCategoria->multiple ? $jugadorSimple->apellidos : $jugadorSimple->apellidos . (preg_match('/\(([^)]+)\)/', $jugadorSimple->nombre_completo_temporal, $matches) ? ' (' . $matches[1] . ')' : '');
                $Jugadores['nombresv2'] = $TorneoCategoria->multiple ? $jugadorDupla->nombres : null;
                $Jugadores['apellidosv2'] = $TorneoCategoria->multiple ? $jugadorDupla->apellidos : null;

                if (preg_match('/\(([^)]+)\)/', $jugadorSimple->nombre_completo_temporal, $matches)) {
                    $jugadoresConNumero[] = $Jugadores;
                } else {
                    $jugadoresSinNumero[] = $Jugadores;
                }
                }

            // Ordenar los jugadores con números por el valor numérico dentro del paréntesis
            ...
            usort($jugadoresSinNumero, function ($a, $b) {
                return strcmp($a['apellidosv1'] . $a['nombresv1'], $b['apellidosv1'] . $b['nombresv1']);
            });

            // Combinar los arrays ordenados
            $model->jugadores = array_merge($jugadoresConNumero, $jugadoresSinNumero);
    ...
    public function exportGrupoJsonVs(Request $request)
{
    try {
        $torneo_categoria_id = $request->categoria;
        $grupo_id =  $request->grupo;
        
        // Obtener la categoría del torneo
        $torneoCategoria = TorneoCategoria::with(['torneo', 'categoriaSimple'])
            ->findOrFail($torneo_categoria_id);
        
        // Obtener el grupo
        $grupo = Grupo::findOrFail($grupo_id);
        
        // Método para obtener imagen del jugador
        $obtenerImagenJugador = function($jugador_id) {
            $jugador_imagen = "";
            ...
            if ($jugador_id) {
                ...
                if ($jugador) {
                    // Verificar si existe imagen personalizada
                    if (Storage::disk('public')->exists("uploads/img/jugador_{$jugador_id}.png")) {
                        $jugador_imagen = Storage::url("uploads/img/jugador_{$jugador_id}.png");
                    } else {
                        // Asignar imagen por defecto según sexo
                        switch ($jugador->sexo) {
                            case 'M':
                                $jugador_imagen = asset("images/hombre.png");
                                break;
                            case 'F':
                                $jugador_imagen = asset("images/mujer.png");
                                break;
                            default:
                                $jugador_imagen = asset("images/incognito.png");
                                break;
                        }
                    }
                } else {
            ...
            return $jugador_imagen;
        };
        
        // Recuperar los partidos del grupo
        $partidos = Partido::where('torneo_categoria_id', $torneo_categoria_id)
            ...
            ->get()
            ->map(function($partido) use ($torneoCategoria, $obtenerImagenJugador) {
                // Preparar información de jugadores
                $jugadoresLocal = $torneoCategoria->multiple
                    ? [
                        'v1' => [
                            'nombres' => $partido->jugadorLocalUno->nombres,
                            'apellidos' => $partido->jugadorLocalUno->apellidos,
                            'imagen' => $obtenerImagenJugador($partido->jugador_local_uno_id),
                            'nombre_completo_temporal' => $partido->jugadorLocalUno->nombre_completo_temporal,
                        ],
                        'v2' => [
                            'nombres' => $partido->jugadorLocalDos->nombres,
                            'apellidos' => $partido->jugadorLocalDos->apellidos,
                            'imagen' => $obtenerImagenJugador($partido->jugador_local_dos_id),
                            'nombre_completo_temporal' => $partido->jugadorLocalDos->nombre_completo_temporal,
                        ]
                    ]
                    : [
                        'v1' => [
                ...
                $jugadoresRival = $torneoCategoria->multiple
                    ? [
                        'v1' => [
                            'nombres' => $partido->jugadorRivalUno->nombres,
                            'apellidos' => $partido->jugadorRivalUno->apellidos,
                            'imagen' => $obtenerImagenJugador($partido->jugador_rival_uno_id),
                            'nombre_completo_temporal' => $partido->jugadorRivalUno->nombre_completo_temporal,
                        ],
                        'v2' => [
                            'nombres' => $partido->jugadorRivalDos->nombres,
                            'apellidos' => $partido->jugadorRivalDos->apellidos,
                            'imagen' => $obtenerImagenJugador($partido->jugador_rival_dos_id),
                            'nombre_completo_temporal' => $partido->jugadorRivalDos->nombre_completo_temporal,
                        ]
                    ]
                    : [
                        'v1' => [
                ...
                return [
                    ...
                    'fecha_final' => $partido->fecha_final,
                    'resultado' => $partido->resultado,
                    'estado' => $partido->estado->nombre,
                    'jugadores' => [
                        'local' => $jugadoresLocal,
                        'rival' => $jugadoresRival
                    ],
                    'sets' => [
                        'local' => $partido->jugador_local_set,
                        'rival' => $partido->jugador_rival_set
                    ],
                    'games' => [
                        'local' => $partido->jugador_local_juego,
                        'rival' => $partido->jugador_rival_juego
                    ]
                ];
            });

        // Preparar el modelo de exportación
        $model = (object)[
            'titulo' => 'La Confraternidad del Tenis',
            'torneo' => $torneoCategoria->torneo->nombre,
            'formato' => $torneoCategoria->torneo->formato?->nombre,
            'multiple' => $torneoCategoria->multiple,
            'categoria' => $torneoCategoria->multiple && ($torneoCategoria->categoriaSimple->id !== $torneoCategoria->categoriaDupla->id)
                ? ($torneoCategoria->categoriaSimple->nombre . " + " . $torneoCategoria->categoriaDupla->nombre)
        ...
        $filename = 'partidos_grupo_vs_' . 
            $torneoCategoria->torneo->id . '_' . 
            $torneo_categoria_id . '_' . 
            $grupo_id . '_' . 
            date('YmdHis') . 
            '.txt';

        // Guardar el contenido en un archivo
        Storage::disk('public')->put('public/uploads/groups/' . $filename, $content);

        // Devolver la URL del archivo JSON
        return redirect(App::$URL_JSON_GROUPS_VS . '?json=' . env('APP_URL') . '/storage/public/uploads/groups/' . $filename);

    } catch (\Exception $e) {
        // Manejar errores
        return response()->json([
            'success' => false,
            'message' => 'Error al recuperar los partidos: ' . $e->getMessage()
        ], 500);
    }
}
    ...
    public function h2h(Request $request){
      ...
      $categoria_simple_id = $request->torneo_categoria_id != null &&  $request->torneo_categoria_id != 'null' ? TorneoCategoria::where('id', $torneo_categoria_id)->first()->categoria_simple_id : $request->categoria_id ;
      ...
      $torneo = $request->torneo_categoria_id != null && $request->torneo_categoria_id != 'null' ? Torneo::where('id', TorneoCategoria::where('id', $torneo_categoria_id)->first()->torneo_id)->first()->nombre : 'Comparativo de resultados';

     $nombre_categoria = $categoria_simple_id != 'null' && $categoria_simple_id != null  ? Categoria::where('id',$categoria_simple_id)->first()->nombre: null;
     
        $rankingMaxLocal = null;
        $categoriaMaxLocal = null;
        $rankingMaxRival = null;
        $categoriaMaxRival = null;
      
     
if ($categoria_simple_id == null || $categoria_simple_id == 'null') {
    
     DB::table('jugadors')
        ->where('id', $jugador_local_uno_id)
        ->update(['ranking_temporal' => null]);

    $categorias = DB::select("
        SELECT tc.categoria_simple_id
        FROM torneo_categorias tc 
        JOIN categorias c ON tc.categoria_simple_id = c.id 
        AND c.dupla = 0 
        AND c.visible = 1 
        JOIN torneo_jugadors tj ON tj.torneo_categoria_id = tc.id 
        WHERE tj.jugador_simple_id = ? AND tc.deleted_at IS NULL AND tj.deleted_at IS NULL
        GROUP BY tc.categoria_simple_id
    ", [$jugador_local_uno_id]);

    $rankingMinLocal = null;
    $categoriaMinLocal = null;
    ...
    foreach ($categorias as $categoria) {
        ...
        $ranking = $this->rankingsByCategoryId($categoria->categoria_simple_id, true);
        $jugador_local = Jugador::where('id', $jugador_local_uno_id)->first();
        if ($jugador_local->ranking_temporal !=null && ($rankingMinLocal === null || $jugador_local->ranking_temporal < $rankingMinLocal)) {
            $rankingMinLocal = $jugador_local->ranking_temporal;
            $categoriaMinLocal = Categoria::where('id', $categoria->categoria_simple_id)->first()->nombre;
        }
    }

    $categoriasv2 = DB::select("
        SELECT tc.categoria_simple_id
        FROM torneo_categorias tc 
        JOIN categorias c ON tc.categoria_simple_id = c.id 
        AND c.dupla = 0 
        ...
        WHERE tj.jugador_simple_id = ? AND tc.deleted_at IS NULL AND tj.deleted_at IS NULL
        GROUP BY tc.categoria_simple_id
    ", [$jugador_rival_uno_id]);

    $rankingMinRival = null;
    $categoriaMinRival = null;

    foreach ($categoriasv2 as $categoriav2) {
         DB::table('jugadors')
        ->where('id', $jugador_rival_uno_id)
        ->update(['ranking_temporal' => null]);

        $ranking = $this->rankingsByCategoryId($categoriav2->categoria_simple_id, true);
      $jugador_rival = Jugador::where('id', $jugador_rival_uno_id)->first();
        
        if ( $jugador_rival->ranking_temporal !=null &&  ($rankingMinRival === null || $jugador_rival->ranking_temporal < $rankingMinRival)) {
            $rankingMinRival = $jugador_rival->ranking_temporal;
            $categoriaMinRival = Categoria::where('id', $categoriav2->categoria_simple_id)->first()->nombre;
        }
    }

     $rankingMaxLocal = $rankingMinLocal;
    $categoriaMaxLocal = $categoriaMinLocal;
    $rankingMaxRival = $rankingMinRival;
    $categoriaMaxRival = $categoriaMinRival;
    
    if (strpos($categoriaMaxLocal, 'Principiante') !== false) {
        $categoriaMaxLocal = str_replace('Principiante', 'Pri.', $categoriaMaxLocal);
    }

    if (strpos($categoriaMaxRival, 'Principiante') !== false) {
        $categoriaMaxRival = str_replace('Principiante', 'Pri.', $categoriaMaxRival);
    }    
    ...
    ->select(
        ...
        'partidos.fecha_inicio as fecha_inicio_torneo',
        DB::raw("IFNULL(partidos.resultado_timestamp, partidos.fecha_final) as fecha_final_torneo"),
        'partidos.fecha_final as fecha_final_torneo',
        'partidos.resultado_timestamp as resultado_timestamp',
        ...
        if($Partidos != null)
        {
            foreach ($Partidos as $partido) {
                $resultado = [
                    ...
                    'resultado' => $partido->resultado ? $partido->resultado : '' , // Asumiendo que hay un campo resultado en la tabla partidos
            ...
            $content = [
                ...
                'ranking_local' =>  $categoria_simple_id !=null && $categoria_simple_id != 'null' ?  $jugadorDatosLocal->ranking_temporal : $rankingMaxLocal . ' - ' . $categoriaMaxLocal, 
                'ranking_rival' =>  $categoria_simple_id !=null && $categoria_simple_id != 'null' ?  $jugadorDatosRival->ranking_temporal : $rankingMaxRival . ' - ' . $categoriaMaxRival,
                ...
                'torneo' => $torneo,
                'ranking_local_max' => $rankingMaxLocal,
                'ranking_rival_max' => $rankingMaxRival,
                'categoria_local_max' => $categoriaMaxLocal,
                'categoria_rival_max' => $categoriaMaxRival,
                'partidos_local' => $resultados ? $resultados : [] ,
    ...
    public function updateJugadorSimpleId(Request $request)
{
    ...
    try {
        // Verificar que ambos jugadores existen en torneo_jugadors
        $torneoJugadorsOld = TorneoJugador::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where('jugador_simple_id', $request->jugador_simple_id)
            ->get();

        $torneoJugadorsNew = TorneoJugador::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where('jugador_simple_id', $request->nuevo_jugador_simple_id)
            ->get();

        if ($torneoJugadorsNew->isEmpty()) {
            DB::rollBack();
            return response()->json(['message' => 'El nuevo jugador no está registrado en esta categoría'], 400);
        }

        // Validar que ninguno de los jugadores tiene partidos con resultados
        $partidosConResultados = Partido::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where(function ($query) use ($request) {
                $query->where('jugador_local_uno_id', $request->jugador_simple_id)
                      ->orWhere('jugador_rival_uno_id', $request->jugador_simple_id)
                      ->orWhere('jugador_local_uno_id', $request->nuevo_jugador_simple_id)
                      ->orWhere('jugador_rival_uno_id', $request->nuevo_jugador_simple_id);
            })
            ->whereNotNull('resultado')
            ->exists();

        if ($partidosConResultados) {
            DB::rollBack();
            return response()->json(['message' => 'No se puede intercambiar los jugadores porque alguno tiene partidos con resultados'], 400);
        }

        // Intercambiar grupos
        $torneoGruposOld = TorneoGrupo::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where('jugador_simple_id', $request->jugador_simple_id)
            ->get();

        $torneoGruposNew = TorneoGrupo::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where('jugador_simple_id', $request->nuevo_jugador_simple_id)
            ->get();

        foreach ($torneoGruposOld as $grupoOld) {
            $grupoOld->jugador_simple_id = $request->nuevo_jugador_simple_id;
            $grupoOld->save();
        }

        foreach ($torneoGruposNew as $grupoNew) {
            $grupoNew->jugador_simple_id = $request->jugador_simple_id;
            $grupoNew->save();
        }

        // Intercambiar partidos
        $partidosOld = Partido::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where(function ($query) use ($request) {
                $query->where('jugador_local_uno_id', $request->jugador_simple_id)
                      ->orWhere('jugador_rival_uno_id', $request->jugador_simple_id);
            })
        ...
        $partidosNew = Partido::where('torneo_categoria_id', $request->torneo_categoria_id)
            ->where(function ($query) use ($request) {
                $query->where('jugador_local_uno_id', $request->nuevo_jugador_simple_id)
                      ->orWhere('jugador_rival_uno_id', $request->nuevo_jugador_simple_id);
            })
            ->whereNull('resultado')
            ->get();

        // Intercambiar jugadores en partidos antiguos
        foreach ($partidosOld as $partidoOld) {
            if ($partidoOld->jugador_local_uno_id == $request->jugador_simple_id) {
                $partidoOld->jugador_local_uno_id = $request->nuevo_jugador_simple_id;
            }
            if ($partidoOld->jugador_rival_uno_id == $request->jugador_simple_id) {
                $partidoOld->jugador_rival_uno_id = $request->nuevo_jugador_simple_id;
            }
            $partidoOld->save();
        }

        // Intercambiar jugadores en partidos nuevos
        foreach ($partidosNew as $partidoNew) {
            if ($partidoNew->jugador_local_uno_id == $request->nuevo_jugador_simple_id) {
                $partidoNew->jugador_local_uno_id = $request->jugador_simple_id;
            }
            if ($partidoNew->jugador_rival_uno_id == $request->nuevo_jugador_simple_id) {
                $partidoNew->jugador_rival_uno_id = $request->jugador_simple_id;
            }
            $partidoNew->save();
        }
        ...
        return response()->json(['message' => 'Jugadores, grupos y partidos intercambiados correctamente']);
    } catch (\Exception $e) {
        DB::rollBack();
        \Log::error('Error al actualizar jugadores: ' . $e->getMessage());
        return response()->json(['message' => 'Ocurrió un error al actualizar los datos', 'error' => $e->getMessage()], 500);
    }
    }   


public function rankingsPartialViewFama(Request $request)
{
    $Model = Comunidad::where('principal', true)->first();
    ...
    if($Model != null)
    {
                        ...
                        $campeones[] = [
                            ...
                            'torneos' => [
                                [
                                    ...
                                    'anio' => Carbon::parse($ranking->torneo->fecha_inicio)->format('Y'),
                                    'categorias' => [
                                        [
        ...
        switch($type) {
           case 'salon_fama':
    $RankingsResultConPosiciones = [];
    foreach ($RankingsResult as &$categoria) {
        if (isset($categoria->jugadores)) {
            // Ordenar jugadores por puntos y nombre
            $jugadoresOrdenados = collect($categoria->jugadores)
                ->sortBy([
                    ['puntos', 'desc'],
                    ['nombre', 'asc']
                ]);
            ...
            foreach ($jugadoresArray as $index => $jugador) {
                // Si es el primer jugador o el puntaje es diferente al anterior
                if ($ultimoPuntaje === null || $jugador['puntos'] != $ultimoPuntaje) {
                    $posicionActual += $empate;
                    $empate = 0;
                }
                
                $posicionesCalculadas[$index] = $posicionActual;
                $ultimoPuntaje = $jugador['puntos'];
                $empate++;
            }

            // Agregar fotos y posiciones
            $jugadoresConPosicion = array_map(function($jugador, $index) use ($posicionesCalculadas) {
                // Agregar foto del jugador
                $folderPath = 'uploads/img';
                
                // Manejar tanto jugadores individuales como de dupla
                $jugadorId = is_string($jugador['id']) ? 
                    explode('-', $jugador['id'])[0] : 
                    $jugador['id'];
                
                $imagePath = "jugador_{$jugadorId}.png";
                $fullImagePath = "{$folderPath}/{$imagePath}";

                if (Storage::disk('public')->exists($fullImagePath)) {
                    $jugador['foto'] = Storage::url($fullImagePath);
                } else {
                    $jugador['foto'] = Storage::url('uploads/img/default_player.png');
                }

                // Agregar posición
                $jugador['posicion'] = $posicionesCalculadas[$index];

                return $jugador;
            }, $jugadoresArray, array_keys($jugadoresArray));

            $categoria->jugadores = $jugadoresConPosicion;
        }
        
        $RankingsResultConPosiciones[] = $categoria;
    }

    $content = json_encode($RankingsResultConPosiciones);
    break;

            default:
                $content = json_encode($RankingsResult);
        }

        Storage::disk('public')->put('public/uploads/parcial_ranking/json.txt', $content);
        
        return redirect(App::$URL_JSON_SALON_FAMA.'?json='.env('APP_URL').'/storage/public/uploads/parcial_ranking/json.txt');
    }
    else
    {
        abort(404);
    ...
    public function rankingsPartialView(Request $request)
    {
        ...
        if($Model != null)
        {
            ...
            ->whereHas('torneoCategoria.torneo', function($query) use ($request) {
                // Filtrar por carrera si se proporciona
                if($request->has('carrera') && $request->carrera == true){
                    $query->where('carrera', true);
                }
            })
            ->get();
            ...
            foreach ($Categorias as $q)
            {
                ...
                $Object['maestros'] =  $request->maestros;
                if ($request->maestros == "false") {
                    // When maestros is false, get the tournament name
                            ...
                            foreach ($q3->detalles as $q4)
                            {
                                ...
                                if (!in_array($Id, $JugadoresIds))
                                {
                                    ...
                                    $ObjectJugador['nombre'] = $q->dupla ? ($q4->jugadorSimple->nombre_completo . ' + ' . $q4->jugadorDupla->nombre_completo) : $q4->jugadorSimple->nombre_completo;
                                      ...
                                      $ObjectJugador['nombres'] = $q->dupla ? ($q4->jugadorSimple->nombres. ' + ' . $q4->jugadorDupla->nombres) : $q4->jugadorSimple->nombres;
                                        $ObjectJugador['apellido'] = $q->dupla ? ($q4->jugadorSimple->apellidos) : $q4->jugadorSimple->apellidos;
...
switch($type) {
    case 'top_ten':
        ...
        foreach ($RankingsResult as &$categoria) {
            ...
            if ($request->maestros === "true") {
                $jugadoresColeccion = $jugadoresColeccion->filter(function($jugador) {
                    return isset($jugador['considerado_ranking']) && $jugador['considerado_ranking'] == 1;
                });
            }
            ...
            $RankingsResultTopTen[] = $categoria;
        }

        $content = json_encode($RankingsResultTopTen);
        break;
    ...
    default:
        ...
        // Seleccionar la URL base según el tipo
        switch($type) {
            case 'top_ten':
                $urlBase = App::$URL_JSON_PARCIAL_RANKING;
                break;
            case 'lista_ranking':
                $urlBase = App::$URL_JSON_ALL_RANKING;
                break;
            case 'salon_fama':
                $urlBase = App::$URL_JSON_SALON_FAMA;
                break;
            default:
                $urlBase = App::$URL_JSON_PARCIAL_RANKING;
        }
        
        return redirect($urlBase.'?json='.env('APP_URL').'/storage/public/uploads/parcial_ranking/json.txt');
    }
    else
    {
        abort(404);
...
public function resultadoRanking(Request $request)
{
    ...
    try {
        ...
        $TorneoCategoria = TorneoCategoria::where('id', $partido->torneo_categoria_id)
            ->whereHas('torneo', function ($q){
                $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id);
            })->first();
        ...
        if($TorneoCategoria != null)
        {
            ...
            $jugadores = DB::table('torneo_jugadors')
                ->where('torneo_jugadors.torneo_categoria_id', $TorneoCategoria->id)
                ->whereNull('torneo_jugadors.deleted_at')
                ->get();

            // Obtener todos los partidos finalizados de esta categoría
            $partidos = Partido::where('torneo_categoria_id', $TorneoCategoria->id)
                ->where('estado_id', 2) // Estado finalizado
                ->whereNull('deleted_at')
                ->get();

            $partidosv2 = Partido::where('torneo_categoria_id', $TorneoCategoria->id)
                ->whereIn('estado_id', [1,2]) // Estado finalizado
                ->whereNull('deleted_at')
                ->get();
            ...
            // Primera fase: Calcular puntos por victoria/derrota
            foreach($partidos as $partidoActual) {
                // Identificar ganador y perdedor
                $ganadorId = $partidoActual->jugador_ganador_uno_id;
                $perdedorId = $partidoActual->jugador_local_uno_id == $ganadorId 
                    ? $partidoActual->jugador_rival_uno_id 
                    : $partidoActual->jugador_local_uno_id;

                if(!isset($puntosAcumulados[$ganadorId])) {
                    $puntosAcumulados[$ganadorId] = 0;
                }
                if(!isset($puntosAcumulados[$perdedorId])) {
                    $puntosAcumulados[$perdedorId] = 0;
                }

                // Calcular puntos del partido
                $formatosWO = ['WO', 'W.O', 'W.O.', 'W/O', 'WALKOVER'];
                $resultado = strtoupper($partidoActual->resultado);
                $esWO = false;
                foreach($formatosWO as $formato) {
                    if(str_contains($resultado, $formato)) {
                        $esWO = true;
                        break;
                    }
                }
                ...
                if($esWO) {
                    $puntosAcumulados[$ganadorId] += 15;
            }else{
                    $setsGanador = max($partidoActual->jugador_local_set, $partidoActual->jugador_rival_set);
                    $setsPerdedor = min($partidoActual->jugador_local_set, $partidoActual->jugador_rival_set);

                    if($setsGanador == 2 && $setsPerdedor == 0) {
                        $puntosAcumulados[$ganadorId] += 15;
                    } else if($setsGanador == 2 && $setsPerdedor == 1) {
                        $puntosAcumulados[$ganadorId] += 10;
                        $puntosAcumulados[$perdedorId] += 5;
                    }
                }

                \Log::info("Puntos partido {$partidoActual->id} - Ganador ID $ganadorId: {$puntosAcumulados[$ganadorId]}, Perdedor ID $perdedorId: {$puntosAcumulados[$perdedorId]}");
            }

            // Segunda fase: Determinar mejor posición de cada jugador
            foreach($jugadores as $jugador) {
                $mejorPuesto = $this->determinarMejorPuesto($jugador->jugador_simple_id, $partidosv2);
                if($mejorPuesto) {
                    $puntosPorPuesto = $this->getPuntajePorPuesto($mejorPuesto);
                    if(!isset($puntosAcumulados[$jugador->jugador_simple_id])) {
                        $puntosAcumulados[$jugador->jugador_simple_id] = 0;
                    }
                    $puntosAcumulados[$jugador->jugador_simple_id] += $puntosPorPuesto;
                    \Log::info("Jugador ID {$jugador->jugador_simple_id} - Puesto: $mejorPuesto, Puntos por puesto: $puntosPorPuesto");
                }
            }

            // Actualizar RankingDetalle para todos los jugadores
            foreach($jugadores as $jugador) {
            ...
            DB::commit();
            $Result->Success = true;
            $Result->Message = "Ranking actualizado correctamente";
        }else{
            $Result->Message = "La categoría del torneo no se encuentra disponible";
        }
...
private function determinarMejorPuesto($jugadorId, $partidos) {
    ...
    $partidosJugador = $partidos->filter(function($partido) use ($jugadorId) {
        return ($partido->jugador_local_uno_id == $jugadorId || 
                $partido->jugador_rival_uno_id == $jugadorId) &&
                $partido->fase !== null;
    })->sortBy('fase');


    \Log::info("V2 ID: $partidosJugador");

    if($partidosJugador->isEmpty()) {
        return null;
    }

    // Verificar si tiene partido pendiente
    $partidoPendiente = $partidosJugador->first(function($partido) {
        return $partido->estado_id === 1;
    });

    // Obtener partidos finalizados
    $partidosFinalizados = $partidosJugador->where('estado_id', 2);

    // Buscar última victoria en partidos finalizados
    $ultimaVictoria = $partidosFinalizados->first(function($partido) use ($jugadorId) {
        return $partido->jugador_ganador_uno_id == $jugadorId;
    });

      // Buscar última derrota en partidos finalizados
    $ultimaDerrota = $partidosFinalizados->first(function($partido) use ($jugadorId) {
        return $partido->jugador_ganador_uno_id !== null && 
               $partido->jugador_ganador_uno_id != $jugadorId;
    });
    // Si tiene partido pendiente
    if($partidoPendiente) {
        \Log::info("Jugador ID: $jugadorId - Tiene partido pendiente en fase: {$partidoPendiente->fase}");
        
        if($ultimaVictoria) {
            \Log::info("ultimaVictoria->fase ID: $jugadorId - Tiene partido pendiente en fase: {$ultimaVictoria->fase}");

            switch($ultimaVictoria->fase) {
                case 2: return 'SEMIFINAL';   // Ganó semifinal, pendiente final
                case 4: return 'CUARTOS';    // Ganó cuartos, pendiente semifinal
                case 8: return 'OCTAVOS';    // Ganó octavos, pendiente cuartos
                case 16: return 'DIECISEISAVOS'; // Ganó dieciseisavos, pendiente octavos
            }
        }
    } 
    // Si no tiene partidos pendientes
    else {
        // Si tiene victorias
        if($ultimaVictoria) {
            $ultimoPartidoFinalizado = $partidosFinalizados->first();
            \Log::info(message: "CAMPEON: {$ultimoPartidoFinalizado}");

            // Si el último partido fue la final
            if($ultimoPartidoFinalizado->fase == 1) {
                \Log::info(message: "CAMPEON: {$ultimoPartidoFinalizado}");

                return $ultimoPartidoFinalizado->jugador_ganador_uno_id == $jugadorId ? 
                       'CAMPEON' : 'SUBCAMPEON';
            }

            // Si tiene una derrota en una fase menor que su última victoria
            if($ultimaDerrota && $ultimaDerrota->fase < $ultimaVictoria->fase) {
                switch($ultimaDerrota->fase) {
            ...
            // Si no, usar la fase de su última victoria
            switch($ultimaVictoria->fase) {
                case 2: return 'SEMIFINAL';
                case 4: return 'CUARTOS';
                case 8: return 'OCTAVOS';
        ...
        // Si no tiene victorias, ver dónde fue eliminado
        else if($ultimaDerrota) {
            switch($ultimaDerrota->fase) {
                case 1: return 'SUBCAMPEON';
                case 2: return 'SEMIFINAL';
                case 4: return 'CUARTOS';
                case 8: return 'OCTAVOS';
                case 16: return null;
...
private function getPuntajePorPuesto($puesto) {
    if($puesto === null) {
        return 0;
    }

    $puntajes = [
        'CAMPEON' => 1000,    // Ganador de la final
        'SUBCAMPEON' => 700,  // Perdedor de la final
        'SEMIFINAL' => 500,   // Eliminado en semifinal
        'CUARTOS' => 200,     // Eliminado en cuartos
        'OCTAVOS' => 100,     // Eliminado en octavos
        'DIECISEISAVOS' => 0  // Eliminado en dieciseisavos
    ];

    return $puntajes[$puesto] ?? 0;
}
