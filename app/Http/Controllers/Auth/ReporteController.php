...
use App\Exports\PartidosAtrasadosExport;
    ...
    {       
        foreach ($Torneos as $q) {
            ...
            $Partidos = Partido::where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
                ...
                ->orderBy('id', 'desc')
                ->get();            // Función para verificar si un jugador está clasificado para la segunda fase
            $verificarClasificacionSegundaFase = function($torneoCategoria, $jugadorId) {
                if($torneoCategoria == null) return false;
                
                // Obtener grupos del torneo categoría
                $TorneoGrupos = TorneoGrupo::where('torneo_categoria_id', $torneoCategoria->id)
                    ->select('grupo_id')->groupBy('grupo_id')->get();

                $JugadoresClasificados = [];
                $Clasifican = $torneoCategoria->clasificados ?? 2; // Por defecto 2 si no está definido

                foreach ($TorneoGrupos as $key => $grupo) {
                    // JUGADORES DEL GRUPO
                    ...
                    // Ordenar y tomar los clasificados
                    ...
                    $JugadoresClasificados = array_merge($JugadoresClasificados, $posicionesOrdenadas->toArray());
                }

                // Verificar si el jugador actual está en la lista de clasificados
                foreach ($JugadoresClasificados as $clasificado) {
                    if ($clasificado['jugador_simple_id'] == $jugadorId) {
                        if ($torneoCategoria->multiple) {
                            // Para dobles, verificar ambos jugadores
                            if ($clasificado['jugador_dupla_id'] != null) {
                                return true;
                            }
                        } else {
                            // Para individuales, solo verificar jugador simple
                            return true;
                        }
                    }
                }
            ...
            if(count($Partidos) > 0) {
                foreach ($Partidos as $Partido) {
                    ...
                    // Verificar si está clasificado para la segunda fase
                    $estaClasificadoSegundaFase = false;
                    
                    // Si es un partido de fase de grupos y el torneo está activo, verificar clasificación
                    if($Partido->fase == null && !in_array($q->estado_id, [App::$ESTADO_CANCELADO, App::$ESTADO_FINALIZADO])) {
                        $estaClasificadoSegundaFase = $verificarClasificacionSegundaFase($Partido->torneoCategoria, $Jugador->id);
                    }
                    ...
                    $HistorialTorneos[] = [
                        ...
                        'Estado' => in_array($q->estado_id, [App::$ESTADO_CANCELADO, App::$ESTADO_FINALIZADO]) ? 'Participación terminada' : ($Partido->estado_id == App::$ESTADO_PENDIENTE ? 'Participación en curso' : ($estaClasificadoSegundaFase ? 'Participación en curso' : ($PartidoNext == null ? 'Participación terminada' : ($PartidoNext->estado_id == App::$ESTADO_PENDIENTE ? 'Participación en curso' : 'Participación terminada'))))
                    ];
    ...
    public function torneoExportarPdf($torneo, $categoria)
{
    ...
    public function jugadoresClasificadosExportarPdf($torneo, $categoria)
    {
        $TorneoCategoria = TorneoCategoria::where('id', $categoria)
            ->where('torneo_id', $torneo)
            ->whereHas('torneo', function ($q) {
                $q->where('comunidad_id', Auth::guard('web')->user()->comunidad_id);
            })->first();

        if ($TorneoCategoria == null) return null;

        $TorneoGrupos = $TorneoCategoria->torneo->torneoGrupos()
            ->where('torneo_categoria_id', $TorneoCategoria->id)
            ->select('grupo_id')
            ->groupBy('grupo_id')
            ->get();
        ...
        foreach ($TorneoGrupos as $key => $grupoRow) {
            // Jugadores del grupo
            $Jugadores = $TorneoCategoria->torneo->torneoGrupos()
                ->where('torneo_categoria_id', $TorneoCategoria->id)
                ->where('grupo_id', $grupoRow->grupo_id)
                ->get()
                ->map(function ($q) use ($TorneoCategoria) {
                    return [
                        'jugador_simple_id' => $q->jugadorSimple->id,
                        'jugador_dupla_id' => $TorneoCategoria->multiple ? $q->jugadorDupla->id : null,
                        'nombres' => $TorneoCategoria->multiple
                            ? ($q->jugadorSimple->nombre_completo . ' + ' . $q->jugadorDupla->nombre_completo)
                            : $q->jugadorSimple->nombre_completo,
                        'grupo_nombre' => $q->grupo->nombre,
                    ];
                });

            $TablePositions = [];

            foreach ($Jugadores as $key2 => $j) {
                if ($TorneoCategoria->multiple) {
                    $PartidosComoLocal = collect($TorneoCategoria->torneo->partidos()
                        ->where('torneo_categoria_id', $TorneoCategoria->id)
                        ->where('grupo_id', $grupoRow->grupo_id)
                        ->where('jugador_local_uno_id', $j['jugador_simple_id'])
                        ->where('jugador_local_dos_id', $j['jugador_dupla_id'])
                        ->whereNull('fase')->get());

                    $PartidosComoRival = collect($TorneoCategoria->torneo->partidos()
                        ->where('torneo_categoria_id', $TorneoCategoria->id)
                        ->where('grupo_id', $grupoRow->grupo_id)
                        ->where('jugador_rival_uno_id', $j['jugador_simple_id'])
                        ->where('jugador_rival_dos_id', $j['jugador_dupla_id'])
                        ->whereNull('fase')->get());

                    $SetsGanados = 0; $SetsPerdidos = 0; $GamesGanados = 0; $GamesPerdidos = 0; $Puntos = 0;

                    foreach ($PartidosComoLocal as $p) {
                        if ($p->jugador_ganador_uno_id == $j['jugador_simple_id']) {
            ...
            $Sorted = App::multiPropertySort(collect($TablePositions), [
                ['column' => 'puntos', 'order' => 'desc'],
                ['column' => 'setsDiferencias', 'order' => 'desc'],
                ['column' => 'gamesDiferencias', 'order' => 'desc'],
                ['column' => 'setsGanados', 'order' => 'desc'],
        ...
        // Clasificación final por reglas actuales
        $PrimerosLugares = []; $SegundoLugares = []; $TercerosLugares = [];
        foreach ($TablePositionsPorGrupo as $value) {
            if ($Clasifican == 1) {
                $PrimerosLugares[] = $value->first();
            } elseif ($Clasifican == 2) {
                $PrimerosLugares[] = $value->first();
                $SegundoLugares[] = $value->get(1);
            } else { // 3
                $PrimerosLugares[] = $value->first();
                $TercerosLugares[] = $value->get(2);
            }
        }

        if ($Clasifican == 3) {
            $clasificadosKeys = array_merge(
                collect($PrimerosLugares)->pluck('key')->toArray(),
                collect($TercerosLugares)->pluck('key')->toArray()
            );
            $SegundoLugares = [];
            foreach ($TablePositionsPorGrupo as $value) {
                foreach ($value as $v) {
                    if (!in_array($v['key'], $clasificadosKeys) && $v === $value->get(1)) {
                        $SegundoLugares[] = $v;
                    }
                }
            }
        ...
        $JugadoresClasificadosMerge = $PrimerosLugares->merge($SegundoLugares)->merge($TercerosLugares);
        $ClasificadosFinal = App::multiPropertySort(collect($JugadoresClasificadosMerge), [
            ['column' => 'puntos', 'order' => 'desc'],
            ['column' => 'gamesDiferencias', 'order' => 'desc']
        ]);

        // No clasificados (todos los que no estén en $ClasificadosFinal)
        $clasificadosKeys = collect($ClasificadosFinal)->pluck('key')->toArray();
        $NoClasificados = [];
        foreach ($TablePositionsPorGrupo as $value) {
            foreach ($value as $v) {
                if (!in_array($v['key'], $clasificadosKeys)) $NoClasificados[] = $v;
            }
        }
        $NoClasificados = App::multiPropertySort(collect($NoClasificados), [
            ['column' => 'puntos', 'order' => 'desc'],
            ['column' => 'gamesDiferencias', 'order' => 'desc']
        ]);
        ...
        $TorneoFaseFinal = (object) [
            'TorneoCategoria' => $TorneoCategoria,
            'JugadoresClasificados' => $ClasificadosFinal,
            'JugadoresNoClasificados' => $NoClasificados,
        ];

        $pdf = Pdf::loadView('auth.torneo.ajax.final.jugadores_clasificados_pdf', [
            'TorneoFaseFinal' => $TorneoFaseFinal
        ])->setPaper('a4', 'portrait');

        return $pdf->stream('JugadoresClasificadosNoClasificados.pdf');
    }
...
public function reportePartidosExcel($torneoId, $categoriaId)
{
    $torneo = Torneo::findOrFail($torneoId);
    $categoria = TorneoCategoria::findOrFail($categoriaId);
    
    // Calcular semanas transcurridas desde el inicio del torneo
    $fechaInicio = \Carbon\Carbon::parse($torneo->fecha_inicio);
    $fechaActual = \Carbon\Carbon::now();
    $semanasTranscurridas = $fechaInicio->diffInWeeks($fechaActual);
    
    // Si el torneo apenas comenzó, considerar al menos 1 semana
    if($semanasTranscurridas == 0 && $fechaActual->greaterThan($fechaInicio)) {
        $semanasTranscurridas = 1;
    }
    
    // Obtener todos los jugadores inscritos en esta categoría del torneo
    $jugadoresInscritos = TorneoJugador::where('torneo_id', $torneoId)
        ->where('torneo_categoria_id', $categoriaId)
        ->with(['jugadorSimple', 'torneoCategoria.categoriaSimple'])
        ->get();
    
    $datos = [];
    
    foreach ($jugadoresInscritos as $torneoJugador) {
        $jugador = $torneoJugador->jugadorSimple;
        
        // Obtener el grupo del jugador
        $torneoGrupo = TorneoGrupo::where('torneo_id', $torneoId)
            ->where('torneo_categoria_id', $categoriaId)
            ->where('jugador_simple_id', $jugador->id)
            ->first();
            
        $nombreGrupo = $torneoGrupo ? $torneoGrupo->nombre_grupo : 'Sin grupo';
        
        // Contar partidos jugados (finalizados) por el jugador en esta categoría
        $partidosJugados = Partido::where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
        ...
        // Contar partidos pendientes
        $partidosPendientes = Partido::where('comunidad_id', Auth::guard('web')->user()->comunidad_id)
            ->where('torneo_id', $torneoId)
            ->where('torneo_categoria_id', $categoriaId)
            ->where('estado_id', App::$ESTADO_PENDIENTE)
            ->where(function ($q) use ($jugador) {
        ...
        $diferencia = $semanasTranscurridas - $partidosJugados;
        $estado = 'Al día'; // Verde
        $colorCodigo = 'Verde';
        
        if($diferencia == 1) {
            $estado = 'Atrasado 1 partido';
            $colorCodigo = 'Amarillo';
        } elseif($diferencia > 1) {
            $estado = 'Muy atrasado (' . $diferencia . ' partidos)';
            $colorCodigo = 'Rojo';
        }
        
        // Solo incluir jugadores que están atrasados (amarillo o rojo)
        if($colorCodigo != 'Verde') {
            $datos[] = [
                'Jugador' => $jugador->nombre_completo,
                'Categoria' => $torneoJugador->torneoCategoria->categoriaSimple->nombre ?? 'Sin categoría',
                'Grupo' => $nombreGrupo,
                'Semanas Transcurridas' => $semanasTranscurridas,
                'Partidos Jugados' => $partidosJugados,
                'Partidos Pendientes' => $partidosPendientes,
                'Partidos Atrasados' => $diferencia,
                'Estado' => $estado,
                'Color' => $colorCodigo,
                'Porcentaje Cumplimiento' => $semanasTranscurridas > 0 ? round(($partidosJugados / $semanasTranscurridas) * 100, 1) . '%' : '0%',
                'Fecha Inicio Torneo' => $torneo->fecha_inicio_texto
            ];
        }
    }
    
    // Ordenar por estado (rojo primero, luego amarillo) y luego por partidos atrasados
    usort($datos, function($a, $b) {
        if($a['Color'] == $b['Color']) {
            return $b['Partidos Atrasados'] - $a['Partidos Atrasados'];
        }
        return ($a['Color'] == 'Rojo') ? -1 : 1;
    });
    
    // Si no hay jugadores atrasados, crear un mensaje informativo
    if (empty($datos)) {
        $datos[] = [
            'Jugador' => 'NINGÚN JUGADOR ATRASADO',
            'Categoria' => $categoria->categoriaSimple->nombre,
            'Grupo' => '-',
            'Semanas Transcurridas' => $semanasTranscurridas,
            'Partidos Jugados' => 'TODOS AL DÍA',
            'Partidos Pendientes' => '-',
            'Partidos Atrasados' => 0,
            'Estado' => 'TODOS LOS JUGADORES ESTÁN AL DÍA CON SUS PARTIDOS',
            'Color' => 'Verde',
    ...
    return Excel::download(
        new PartidosAtrasadosExport($datos, $torneo->nombre, $categoria->categoriaSimple->nombre), 
        'jugadores_atrasados_' . Str::slug($torneo->nombre) . '_' . Str::slug($categoria->categoriaSimple->nombre) . '_' . date('Y-m-d') . '.xlsx'
    );
}

private function obtenerNombreGanador($partido, $esMultiple)
{
    if (!$partido->jugador_ganador_uno_id) return '';
    
    if ($esMultiple) {
        $ganadorUno = Jugador::find($partido->jugador_ganador_uno_id);
        $ganadorDos = Jugador::find($partido->jugador_ganador_dos_id);
        return $ganadorUno->nombre_completo . ' + ' . $ganadorDos->nombre_completo;
    } else {
        $ganador = Jugador::find($partido->jugador_ganador_uno_id);
        return $ganador->nombre_completo_temporal;
    }
}

private function calcularDiasVencido($partido)
{
    if (!$partido->fecha_final || $partido->estado_id == App::$ESTADO_FINALIZADO) {
        return '';
    }
    
    $fechaFinal = Carbon::parse($partido->fecha_final);
    $hoy = Carbon::now();
    
    if ($hoy->gt($fechaFinal)) {
        return $hoy->diffInDays($fechaFinal) . ' días';
    }
