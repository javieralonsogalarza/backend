<div class="modal fade" id="modal{{$ViewName}}" role="dialog" tabindex="-1" data-backdrop="static"
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('auth.' . strtolower($ViewName) . '.store') }}" id="frm{{$ViewName}}"
                enctype="multipart/form-data" method="POST" data-ajax="true" class="form" data-ajax-loading="#loading"
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <div class="col-md-4">
                            ...
                            <div class="image_preview_content" style="height: 230px">
                                <div class="image_preview">
                                    <img src="{{ ($Model != null && $Model->imagen_path != null && $Model->imagen_path != "") ? ('/img/' . $Model->imagen_path) : "/upload/image/default.png" }}"
                                        alt="Logo">
                                </div>
                            </div>
                            <input type="file" class="preview form-control" name="imagen" id="imagen"
                        ...
                        <div class="col-md-8">
                            <div class="form-group row">
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" name="nombres" id="nombres" class="form-control"
                                        value="{{ $Model != null ? $Model->nombres : "" }}" required autocomplete="off">
                                    <span data-valmsg-for="nombres"></span>
                                </div>
                                <div class="col-sm-6">
                                    <label for="apellidos">Apellidos: <span class="text-danger">(*)</span></label>
                                    <input type="text" name="apellidos" id="apellidos" class="form-control"
                                        value="{{ $Model != null ? $Model->apellidos : "" }}" required
                                        autocomplete="off">
                                    ...
                                    <div class="input-group">
                                        <select name="categoria_id" id="categoria_id" class="form-control">
                                            ...
                                            @foreach($Categorias as $q)
                                                <option value="{{ $q->id }}" {{ $Model != null && $Model->categoria_id == $q->id ? "selected" : "" }}>{{ $q->nombre }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <span class="input-group-append">
                                            <button type="button" id="btnRegistrarCategoria" class="btn btn-primary"><i
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    <label for="tipo_documento_id">Tipo documento: <span
                                            class="text-danger">(*)</span></label>
                                    <select name="tipo_documento_id" id="tipo_documento_id" class="form-control"
                                        required>
                                        <option value="">Ninguno</option>
                                        @foreach($TipoDocumentos as $q)
                                            <option value="{{ $q->id }}" {{ $Model != null && $Model->tipo_documento_id == $q->id ? "selected" : "" }}>{{ $q->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-sm-6">
                                    <label for="nro_documento">Nº documento: <span
                                            class="text-danger">(*)</span></label>
                                    <input type="text" name="nro_documento" id="nro_documento" class="form-control"
                                        value="{{ $Model != null ? $Model->nro_documento : "" }}" autocomplete="off"
                                        required>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="input-group">
                                        <input type="email" name="email" id="email" class="form-control"
                                            value="{{ $Model != null ? $Model->email : "" }}" autocomplete="off">
                                        @if($Model != null)
                                            <span class="input-group-append">
                                                <button type="button" id="btnCrearCuenta" class="btn btn-success"
                                                    title="{{ $Model->isAccount ? '¿Actualizar Cuenta?' : '¿Crear Cuenta?'  }}">
                                                    @if($Model->isAccount)
                                                ...
                                                @if($Model->isAccount)
                                                    <button type="button" id="btnEliminarCuenta" class="btn btn-danger"
                                                        title="Eliminar Cuenta"><i class="fa fa-ban"></i></button>
                                                @endif
                                            </span>
                                        @endif
                                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-md-4">
                            ...
                            <select name="sexo" id="sexo" class="form-control" required>
                                <option value="" {{ $Model != null && $Model->sexo == null ? "selected" : "" }}>Ninguno
                                </option>
                                <option value="M" {{ $Model != null && $Model->sexo == "M" ? "selected" : "" }}>Masculino
                                </option>
                                <option value="F" {{ $Model != null && $Model->sexo == "F" ? "selected" : "" }}>Femenino
                                </option>
                            </select>
                        ...
                        <div class="col-sm-4">
                            ...
                            <input type="date" name="fecha_nacimiento" id="fecha_nacimiento" class="form-control"
                                value="{{ $Model != null ? $Model->fecha_nacimiento : "" }}" autocomplete="off">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <select name="marca_raqueta" id="marca_raqueta" class="form-control">
                                    ...
                                    <option value="Wilson" {{ $Model != null && $Model->marca_raqueta == 'Wilson' ? 'selected' : '' }}>Wilson</option>
                                    <option value="Babolat" {{ $Model != null && $Model->marca_raqueta == 'Babolat' ? 'selected' : '' }}>Babolat</option>
                                    <option value="Head" {{ $Model != null && $Model->marca_raqueta == 'Head' ? 'selected' : '' }}>Head</option>
                                    <option value="Yonex" {{ $Model != null && $Model->marca_raqueta == 'Yonex' ? 'selected' : '' }}>Yonex</option>
                                    <option value="Prince" {{ $Model != null && $Model->marca_raqueta == 'Prince' ? 'selected' : '' }}>Prince</option>
                                    <option value="Dunlop" {{ $Model != null && $Model->marca_raqueta == 'Dunlop' ? 'selected' : '' }}>Dunlop</option>
                                    <option value="Tecnifibre" {{ $Model != null && $Model->marca_raqueta == 'Tecnifibre' ? 'selected' : '' }}>Tecnifibre</option>
                                    <option value="Otros" {{ $Model != null && $Model->marca_raqueta == 'Otros' ? 'selected' : '' }}>Otros</option>
                                </select>
                            </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-4">
                            ...
                            <input type="text" name="altura" id="altura" class="form-control decimal-height"
                                value="{{ $Model != null ? $Model->altura : "" }}" autocomplete="off">
                            <span data-valmsg-for="altura"></span>
                        </div>
                        <div class="col-sm-4">
                            <label for="peso">Peso (kg): </label>
                            <input type="text" name="peso" id="peso" class="form-control decimal-weight"
                                value="{{ $Model != null ? $Model->peso : "" }}" autocomplete="off">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="mano_habil" id="mano_habil" class="form-control">
                                ...
                                <option value="Diestra" {{ $Model != null && $Model->mano_habil == 'Diestra' ? 'selected' : '' }}>Diestra</option>
                                <option value="Zurda" {{ $Model != null && $Model->mano_habil == 'Zurda' ? 'selected' : '' }}>Zurda</option>
                                <option value="Ambidiestra" {{ $Model != null && $Model->mano_habil == 'Ambidiestra' ? 'selected' : '' }}>Ambidiestra</option>
                            </select>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit"
                        class="btn btn-primary pull-right">{{ ($Model != null ? "Modificar" : "Registrar") }}</button>
                </div>
            </form>
        </div>
    </div>
